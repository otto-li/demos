...
-- MAGIC %md
-- MAGIC Your DLT Pipeline has been installed and started for you! Open the <a dbdemos-pipeline-id="dlt-iot-wind-turbine" href="#joblist/pipelines/174b7f11-f421-498b-9c31-3b538de9fcad" target="_blank">IOT Wind Turbine Delta Live Table pipeline</a> to see it in action.<br/>
-- MAGIC
...
-- display(spark.read.json('/Volumes/otto/iot/turbine_raw_landing/turbine'))
-- display(spark.read.json('/Volumes/otto/iot/turbine_raw_landing/historical_turbine_status')) #Historical turbine status analyzed

-- COMMAND ----------

-- DBTITLE 1,Wind Turbine sensor data
-- %python #uncomment to scan the data from the notebook
-- display(spark.read.parquet('/Volumes/otto/iot/turbine_raw_landing/incoming_data'))
...
AS SELECT * FROM cloud_files("/Volumes/otto/iot/turbine_raw_landing/turbine", "json", map("cloudFiles.inferColumnTypes" , "true"))
...
AS SELECT * FROM cloud_files("/Volumes/otto/iot/turbine_raw_landing/incoming_data", "parquet", map("cloudFiles.inferColumnTypes" , "true"))
...
AS SELECT * FROM cloud_files("/Volumes/otto/iot/turbine_raw_landing/historical_turbine_status", "json", map("cloudFiles.inferColumnTypes" , "true"))

-- COMMAND ----------

CREATE STREAMING TABLE parts 
COMMENT "Turbine parts from our manufacturing system"
AS SELECT * FROM cloud_files("/Volumes/otto/iot/turbine_raw_landing/parts", "json", map("cloudFiles.inferColumnTypes" , "true"))
...
-- MAGIC
-- MAGIC ## Conclusion
-- MAGIC Our <a dbdemos-pipeline-id="dlt-iot-wind-turbine" href="#joblist/pipelines/174b7f11-f421-498b-9c31-3b538de9fcad" target="_blank">DLT Data Pipeline</a> is now ready using purely SQL. We have an end 2 end cycle, and our ML model has been integrated seamlessly by our Data Engineering team.
-- MAGIC
-- MAGIC
...
-- MAGIC
-- MAGIC We are now ready to build our <a dbdemos-dashboard-id="turbine-analysis" href="/sql/dashboardsv3/01ef5e992318104dbc71e440f1900e40">DBSQL Dashboard</a>to track the main KPIs and status of our entire Wind Turbine Farm and build complete <a dbdemos-dashboard-id="turbine-predictive" href="/sql/dashboardsv3/01ef5e992318104dbc71e440f1900e40">Predictive maintenance DBSQL Dashboard</a>.
-- MAGIC
-- MAGIC
